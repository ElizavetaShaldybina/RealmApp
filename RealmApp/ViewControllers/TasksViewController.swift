...
extension TasksViewController {
        ...
        alertBuilder
            ...
            .addAction(
                ...
                if let task, let completion {
                    self?.storageManager.edit(task: task, title: taskTitle, note: taskNote)
                    completion()
                    return
                }
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let doneAction = UIContextualAction(style: .normal, title: buttonTitle) { [unowned self] _, _, isDone in
            ...
            let currentTask = IndexPath(row: self.currentTasks.count - 1, section: 0)
            let comletedTask = IndexPath(row: self.completedTasks.count - 1, section: 1)
            
            let taskPlace = indexPath.section == 0 ? comletedTask : currentTask
            
            tableView.moveRow(at: indexPath, to: taskPlace)
